...
def compute_shap_values(model, X: pd.DataFrame, feature_cols: list) -> pd.DataFrame: 
    """ Runs SHAP's TreeExplainer against the trained XGBoost model. Returns one row per sample, one column per feature, values = each feature's contribution (positive = pushed AQI up, negative = pushed it down). """ 
    explainer = shap.TreeExplainer(model) 
    shap_values = explainer.shap_values(X) 
    return pd.DataFrame(shap_values, columns=feature_cols) 

def get__top_drivers(shap_df: pd.DataFrame, n: int = 3) -> list: 
    """ Ranks features by absolute SHAP impact on the most recent row and returns the top N as (feature_name, direction, magnitude) tuples. """ 
    last_row = shap_df.iloc[-1] 
    ranked = last_row.reindex(last_row.abs().sort_values(ascending=False).index) 
    drivers = [] 
    for feature, value in ranked.head(n).items(): direction = "increasing" if value > 0 else "decreasing" 
    drivers.append((feature, direction, abs(round(value, 2)))) 
    return drivers 
FEATURE_LABELS = { "aqi_lag_1": "AQI one hour ago", "aqi_lag_24": "AQI at this time yesterday", "wind_speed": "wind speed", "humidity": "humidity", "hour_sin": "time of day", "is_weekend": "weekend effect", "aqi_rolling_mean_6": "the 6-hour average trend", } 
...
def generate_nl_explanation(drivers: list, current_aqi: float, forecast_peak: float) -> str: 
    ...
    return (f"AQI is currently {current_aqi:.0f} and is {trend_word} toward a "f"6-hour peak of {forecast_peak:.0f}. The main contributors are: {driver_text}.") 

def classify_trend(forecast_6h: list) -> str: 
    """ Buckets the 6-hour forecast shape into one of three labels the frontend uses for its trend badge and messaging. """ 
    ...
    if forecast_6h[-1] < max(forecast_6h) - 10: 
        return "temporary"          # rises then meaningfully falls back return "fluctuating" i
    if __name__ == "__main__": 
        ...
        with open("models/xgb_feature_cols.pkl", "rb") as f: 
            ...
            drivers = get__top_drivers(shap_df, n=3) 
